...
|---|---|---|
| [May 22 Build/Run design](specs/2026-05-22-nn-forge-build-run-instrument-design.md) | Superseded by the production Precision Lab shell while its experiment and training contracts remain current | The application composes `PrecisionLabShell`; obsolete Build/Run presentation adapters were retired. Profile-driven visibility supersedes universal specialist-panel visibility. Saved recipe application is not parameter restoration. |
...
| [July 16 Precision Lab design](specs/2026-07-16-precision-lab-production-integration-design.md) and [July 17 implementation plan](plans/2026-07-17-precision-lab-production-integration.md) | Implemented as the current production shell; later refinements supersede parts of the dated task sequence | The production shell includes the pinned boundary rail, selection deck, production-backed compact evidence, and responsive integration. Current user-facing terminology is `Workspace profile`; do not blindly restore the historical `Audience mode` label. |
