...
function validateDecodedDocument(
    ...
    const looksUnversioned = isV2Record(value) && (
    ...
    if (isV2Record(value) && value.schemaVersion === 1) {
        return schemaFailure(
            'legacy-state',
            'schemaVersion',
...
export function decodeExperimentUrl(
    ...
    for (const key of parameters.keys()) {
        if (key !== 'v' && key !== 'r') {
            return schemaFailure('unknown-field', key, `URL field ${key} is not allowed`);
        }
    }
    ...
    if (payloads.length !== 1) {
        return schemaFailure(
            'invalid-field',
            'r',
            'URL must contain exactly one recipe payload',
        );
    }
