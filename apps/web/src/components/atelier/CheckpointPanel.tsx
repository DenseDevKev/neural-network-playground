import '../controls/exportUtilities.css';
...
export function CheckpointPanel({ training }: { training: TrainingHook }) {
    ...
    const current = useTrainingStore((s) => selectScientificEvidence(s).currentModel?.step ?? 0);
    ...
    return <section className="atelier-checkpoints">
        ...
        <input id="atelier-checkpoint-range" type="range" min={0} max={timeline.checkpoints.length - 1} step={1} value={index} disabled={busy} aria-valuetext={selected.label} onChange={(event) => setId(timeline.checkpoints[Number(event.target.value)].id)} />
        <p>{timeline.restoredCheckpointId === selected.id ? 'Restored checkpoint' : 'Not restored yet'}</p>
        ...
        <div className="atelier-dialog-actions">{status === 'running' && <button type="button" onClick={training.pause}>Pause training</button>}<button type="button" className="atelier-primary" disabled={status === 'running' || pending !== null || busy} onClick={async () => { if (restoring.current || useTrainingStore.getState().status === 'running' || useTrainingStore.getState().pendingConfigSource !== null) return; restoring.current = true; setBusy(true); setError(null); try { await training.restoreCheckpoint(selected.id); } catch (cause) { setError(cause instanceof Error ? cause.message.slice(0, 1000) : 'Checkpoint restoration failed.'); } finally { restoring.current = false; setBusy(false); } }}>{busy ? 'Restoring…' : `Restore ${selected.label}`}</button></div>
    </section>;
}
