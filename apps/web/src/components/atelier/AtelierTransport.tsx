...
export function AtelierTransport({ training, onCheckpoints }: { training: TrainingHook; onCheckpoints(): void }) {
    ...
    return <section className="atelier-transport" aria-label="Training controls" data-lesson-target="transport">
        <div className="atelier-transport-row">
            <button type="button" className="atelier-primary" aria-label={lifecycle.primaryAriaLabel} disabled={!running && lifecycle.isBlocked} onClick={running ? training.pause : training.play}><Icon name={running ? 'pause' : 'play'} />{lifecycle.primaryLabel}</button>
            <button type="button" aria-label="Run one training step" disabled={running || lifecycle.isBlocked} onClick={training.step}><Icon name="step" />Step</button>
            ...
            <div className="atelier-transport-position"><div><span>Step </span><strong>{(model?.step ?? 0).toLocaleString()}</strong></div><div><span>Epoch </span><strong>{model?.epoch ?? 0}</strong></div><button type="button" onClick={onCheckpoints}><Icon name="history" />Checkpoints</button></div>
        </div>
        <p className="atelier-transport-status" role="status">{running ? 'Training · updates run locally in your browser' : lifecycle.statusText ?? 'Ready when you are · choose Play to begin learning'}</p>
    </section>;
}
...
export function EvidenceMetrics() {
    ...
    return <div>
        <dl className="atelier-metrics">
            ...
            {full?.testAccuracy !== undefined ? <div><dt>Test accuracy</dt><dd>{(full.testAccuracy * 100).toFixed(1)}<small>%</small><small>Full held-out evaluation</small></dd></div> : <div><dt>Training objective</dt><dd>{metric(full?.trainingObjective)}<small>Data loss + regularization</small></dd></div>}
